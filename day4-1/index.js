...
const FALLS_ASLEEP = Symbol('FALLS_ASLEEP')
const WAKES_UP = Symbol('WAKES_UP')
const BEGINS_SHIFT = Symbol('BEGINS_SHIFT')

const getActionFromString = x => {
    if(x === 'falls asleep') {
        return {
            action: FALLS_ASLEEP,
            guard: null,
    ...
    else if(x.indexOf('begins shift') !== -1) {
        const result = /^Guard #([0-9]+) begins shift$/.exec(x)
        return {
            action: BEGINS_SHIFT,
            guard: result[1],
        }
...
const lineToObj = line => {
    const result = /^\[([0-9]{4}-[0-9]{2}-[0-9]{2} [0-9]{2}:[0-9]{2})\] (.*)$/.exec(line)

    return {
        ...getActionFromString(result[2]),
        when: parseDate(result[1]),
    }
}

const buildSleepWakePeriods = xs => {
    const periods = []
    let lastGuard = null
    let asleepAt = null

    xs.forEach(x => {
        if(x.action === BEGINS_SHIFT) {
            lastGuard = x.guard
        }
        else if(x.action === FALLS_ASLEEP) {
            asleepAt = x.when
        }
        else if(x.action === WAKES_UP) {
            periods.push({
                guard: lastGuard,
                asleep: asleepAt,
                wake: x.when,
                length: differenceInMinutes(x.when, asleepAt),
            })
        }
    })
...
const getMostAsleep = R.compose(
    R.prop('guard'),
    reduce((acc, length, guard) => {
        return acc === null || length > acc.length
            ? {length, guard}
            : acc
    }, null),
    R.reduce((acc, period) => {
        if(!acc.hasOwnProperty(period.guard)) acc[period.guard] = 0

        acc[period.guard] += period.length

        return acc
    }, {}),
)

const getPopularMinute = (guard, periods) => {
    return R.compose(
        R.prop('minute'),
        reduce((acc, num, minute) => {
        ...
        R.reduce((acc, x) => {
            ...
            while(!isEqual(t, x.wake)) {
                const m = getMinutes(t)

                if(!acc.hasOwnProperty(m)) acc[m] = 0
...
const periods = R.compose(
    buildSleepWakePeriods,
    R.sort((a, b) => compareAsc(a.when, b.when)),
    R.map(lineToObj),
    R.filter(x => !!x),
    R.map(R.trim),
    R.split('\n'),
...
const mostAsleepGuard = getMostAsleep(periods)

const popularMinute = getPopularMinute(mostAsleepGuard, periods)

console.log(parseInt(mostAsleepGuard) * parseInt(popularMinute))
