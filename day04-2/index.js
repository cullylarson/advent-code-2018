...
const {map, reduce} = require('@cullylarson/f')
...
const getMostGuardMinute = (periods) => {
    return R.compose(
        reduce((acc, x, guard) => {
            return acc === null || x.num > acc.num
                ? {...x, guard}
                : acc
        }, null),
        map(reduce((acc, num, minute) => {
        ...
        R.reduce((acc, x) => {
            ...
            while(!isEqual(t, x.wake)) {
                ...
                if(!acc.hasOwnProperty(x.guard)) acc[x.guard] = {}
                if(!acc[x.guard].hasOwnProperty(m)) acc[x.guard][m] = 0
...
const popularGuardMinute = R.compose(
    getMostGuardMinute,
...
console.log(parseInt(popularGuardMinute.minute) * parseInt(popularGuardMinute.guard))
